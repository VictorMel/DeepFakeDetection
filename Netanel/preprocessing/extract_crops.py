...
def extract_video(param, root_dir, crops_dir):
    ...
    for i in range(frames_num):
        ...
        for j, crop in enumerate(crops):
            cv2.imwrite(os.path.join(img_dir, f"{i}_{j}.png"), crop)


def get_video_paths(root_dir):
    paths = []
    for json_path in glob(os.path.join(root_dir, "*/metadata.json")):
        directory = Path(json_path).parent
        with open(json_path, "r") as f:
            metadata = json.load(f)
        for k, v in metadata.items():
            original = v.get("original", None) or k
            bboxes_path = os.path.join(root_dir, "boxes", f"{original[:-4]}.json")
            if not os.path.exists(bboxes_path):
                continue
            paths.append((os.path.join(directory, k), bboxes_path))
...
if __name__ == '__main__':
    ...
    with Pool(processes=cpu_count()) as p:
        with tqdm(total=len(params)) as pbar:
            for _ in p.imap_unordered(partial(extract_video, root_dir=args.root_dir, crops_dir=args.crops_dir), params):
                pbar.update()
