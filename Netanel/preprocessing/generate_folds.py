...
def main():
    ...
    folds = [
        list(
            range(
                sz * fold, sz * fold + sz if fold < args.n_splits - 1 else 50
            )
        )
    ...
    data = []
    ori_ori = {(ori, ori) for ori, fake in ori_fakes}
    ...
    for img_path, label, ori_vid in data:
        ...
        file = path.name
        assert (
            video_fold[video] == video_fold[ori_vid]
        ), f"original video and fake have leak  {ori_vid} {video}"
