...
class VideoReader:
    ...
    def read_frames(self, path, num_frames, jitter=0, seed=None):
        ...
        if jitter > 0:
            np.random.seed(seed)
            jitter_offsets = np.random.randint(-jitter, jitter, len(frame_index))
            frame_index = np.clip(frame_index + jitter_offsets, 0, frame_count - 1)

        result = self._read_frames_at_indices(path, capture, frame_index)
        capture.release()
        return result
    ...
    def read_random_frames(self, path, num_frames, seed=None):
        ...
        frame_index = sorted(np.random.choice(np.arange(0, frame_count), num_frames))
        result = self._read_frames_at_indices(path, capture, frame_index)

        capture.release()
        return result

    def read_frames_at_indices(self, path, frame_index):
        """Reads frames from a video and puts them into a NumPy array.

        Arguments:
            path: the video file
            frame_index: a list of frame indices. Important: should be
    ...
    def _read_frames_at_indices(self, path, capture, frame_index):
        try:
            frames = []
            index_read = []
            for frame_idx in range(frame_index[0], frame_index[-1] + 1):
                ...
                current = len(index_read)
                if frame_idx == frame_index[current]:
                    ...
                    frames.append(frame)
                    index_read.append(frame_idx)

            if len(frames) > 0:
                return np.stack(frames), index_read
            if self.verbose:
                print("No frames read from movie %s" % path)
            return None
        except Exception:
            if self.verbose:
                print(f"Exception while reading movie {path}")
            return None
            ...
            if n > 0:
                ...
                with torch.no_grad():
                    ...
                    for model in models:
                        ...
                        batch_predictions = y_pred[:n].cpu().numpy()
                        predictions.append(strategy(batch_predictions))
...
def predict_on_video_set(face_extractor, videos, input_size, num_workers, test_dir, frames_per_video, models, strategy=np.mean, apply_compression=False):
    def process_file(i):
        filename = videos[i]
        y_pred = predict_on_video(face_extractor, os.path.join(test_dir, filename), input_size, frames_per_video, models, strategy, apply_compression)
        return y_pred
